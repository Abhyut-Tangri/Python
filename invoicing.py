...
def parsing_number(invoice_number:str)->tuple[int,int]:
    """returns invoice in two parts
...
def next_invoice_number(invoice_number: str) -> str:
    ...
    """
    invoice_year, number = parsing_number(invoice_number)
    year = get_year()
...
def record_invoice(invoice_file: TextIO,
    ...
    for line in invoice_file:
        ...
        lastrow=line
    if lastrow:
        invoice_number,_,_=lastrow.split('\t')
...
for test_string, result, next_number in test_data:
    parts = parsing_number(test_string)
    if parts == result:
